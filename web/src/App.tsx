...
const AddNewPayment = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
       ...
       await response.json();
      setMessage(`Payment added successfully! Amount: ${amount} on ${paymentDate}`);
      setLoanId('');
      setAmount('');
      setPaymentDate(''); 
      ...
      if (errorMsg.includes('does not exist')) {
        ...
        friendlyMessage = 'Invalid input — please check Loan ID (number), Amount (positive number), and Date format.';
  ...
  return (
    <div style={{ marginTop: '2rem' }}>
      ...
      {message && (
        <p style={{ color: 'lime', marginTop: '1rem' }}>{message}</p>
      )}
      {error && (
